...
def list_media_devices():
    ...
    with open("/proc/partitions", "r") as f:
        devices = []
        scsi_block_devices_maior_number = (8,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80,81,82,83,84,85,86,87,93,104,105,106,107,108,110,111)
        raid_controller_block_devices_maior_number = (48,49,50,51,52,53,54,55)
        ide_block_devices_maior_number = (3,13,14,21,22,34,36,88,89,90,91)
        block_device_maior_number = scsi_block_devices_maior_number + raid_controller_block_devices_maior_number + ide_block_devices_maior_number
        for line in f.readlines()[2:]:  # skip header lines
            words = [word.strip() for word in line.split()]
            maior_number = int(words[0])
            minor_number = int(words[1])
            device_name = words[3]
            #Check if is a block device using maior number in kernel device.txt
            check_device = False
            if (maior_number in block_device_maior_number):
                check_device = True

                if (maior_number in scsi_block_devices_maior_number):
                    set_minor_number_for_device = 16

                if (maior_number in raid_controller_block_devices_maior_number):
                    set_minor_number_for_device = 8

                if (maior_number in ide_block_devices_maior_number):
                    set_minor_number_for_device = 64

            if (check_device):
                if (minor_number % set_minor_number_for_device) == 0:
                    path = "/sys/class/block/" + device_name

                    if os.path.islink(path):
                        #	if os.path.realpath(path).find("/removable") == 0:
                        print(os.path.realpath(path))
                        #devices.append("/dev/" + device_name)
                        blockdevice = Device("/dev/" + device_name)
...
class Device:
    'Common base class for all device'
    drive = 'null'
    mounted = 'No'
    size = '0'
    model = 'null'
    ...
    def __init__(self, device ):
        self.device = device
        self.drive = get_device_name(device)
        self.mounted = is_mounted(device)
        self.size = get_size(device) / 1024 ** 3
        self.model = get_model(device)
        self.vendor = get_vendor(device)
